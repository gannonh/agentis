...
test.describe('Organization Join Flow - Issue #104', () => {
  ...
  test('Complete organization join flow with domain auto-join', async ({ browser }) => {
    ...
    try {
      ...
      // Create organization with domain join enabled using the test context name
      ...
      const expectedDomain1 = testContext1.emails.corporate!.split('@')[1]; // Extract domain from email
      const domainJoinLabel = page1.getByText(new RegExp(`let anyone with an @${expectedDomain1.replace('.', '\\.')} email join`, 'i'));
      await expect(domainJoinLabel).toBeVisible();
      await page1.getByRole('checkbox').check();
      logProgress(`✅ User 1: Enabled domain join for @${expectedDomain1}`);
      ...
      await expect(
        page2.getByText(
          new RegExp(`You can automatically join this organization with your ${sharedDomain.replace('.', '\\.')} email`),
        ),
      ).toBeVisible();
      logProgress(`✅ User 2: Can see domain information for ${sharedDomain}`);
      ...
      expect(org?.metadata?.domain).toBe(expectedDomain1); // Use the same domain we extracted earlier
