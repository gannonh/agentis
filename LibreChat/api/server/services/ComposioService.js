...
class ComposioService {
  ...
  async getOrCreateConnectedAccount(userId, service) {
    ...
    if (existingDbRecord) {
      ...
      if (timeSinceLastCheck < 5 * 60 * 1000) {
        logger.info(`[ComposioService] Using recently verified connection ${existingDbRecord.connectedAccountId} for user ${userId}, service ${service}`);
    ...
    try {
      ...
      if (activeConnection) {
        logger.info(`[ComposioService] Found existing ACTIVE connection for user ${userId}, service ${service}, connection: ${activeConnection.id}`);
      ...
      if (existingConnection) {
        logger.info(`[ComposioService] Found existing connection for user ${userId}, service ${service}, connection: ${existingConnection.id}, status: ${existingConnection.status}`);
        
        // If connection is not ACTIVE, try to check if it became active recently
        if (existingConnection.status !== 'ACTIVE') {
          logger.info(`[ComposioService] Connection is ${existingConnection.status}, checking if it became active...`);
          ...
          try {
            ...
            const updatedConnection = updatedConnections.find(conn => conn.id === existingConnection.id);
            ...
            if (updatedConnection && updatedConnection.status === 'ACTIVE') {
              ...
              logger.info(`[ComposioService] Connection ${existingConnection.id} is still ${updatedConnection?.status || 'not found'}, but will use it anyway since tools work`);
            }
          } catch (error) {
      ...
      logger.warn(`[ComposioService] Failed to check existing connections for user ${userId}, service ${service}:`, error);
    }

    // Only create new connection if no existing connections found
    logger.info(`[ComposioService] No existing connections found for user ${userId}, service ${service} - creating new one`);

    // Create new connected account via Composio API
    try {
      logger.info(`[ComposioService] Creating new connected account for user ${userId}, service ${service}, app ${appName}`);

      // Use the SDK method but without trying to override callback URL
      logger.info(`[ComposioService] Creating entity for userId: "${userId}" (type: ${typeof userId})`);
      const entity = this.composio.getEntity(userId);
      ...
      logger.info(`[ComposioService] Created connected account ${response.connectedAccountId} for user ${userId}, service ${service}`);

      return response.connectedAccountId;
    } catch (error) {
      logger.error(`[ComposioService] Failed to create connected account for user ${userId}, service ${service}:`, error);
      throw error;
    }
  }
  ...
  async cleanupAndStartFresh(userId, service) {
    ...
    try {
      ...
      try {
        ...
        for (const conn of serviceConnections) {
          try {
            ...
            logger.info(`[ComposioService] Deleted user connection ${conn.id} for user ${userId}, app ${appName}`);
          } catch (error) {
      ...
      try {
        ...
        const defaultServiceConnections = defaultConnections.filter(conn => 
          conn.appName.toLowerCase() === appName.toLowerCase()
        );
        ...
        for (const conn of defaultServiceConnections) {
          try {
            ...
            logger.info(`[ComposioService] Deleted default connection ${conn.id} for app ${appName}`);
          } catch (error) {
      ...
      logger.error(`[ComposioService] Failed to cleanup for user ${userId}, service ${service}:`, error);
    }
  }
  ...
  async validateConnectedAccount(connectedAccountId) {
    ...
    try {
      ...
      logger.error(`[ComposioService] Failed to validate connected account ${connectedAccountId}:`, error);
      return false;
    }
  }
  ...
  async hasActiveConnection(userId, service) {
    ...
    try {
      ...
      logger.warn(`[ComposioService] Failed to check connection status for user ${userId}, service ${service}:`, error);
      return false;
    }
  }
  ...
  async initiateConnection(userId, service) {
    ...
    try {
      ...
      logger.info(`[ComposioService] Initiating OAuth connection for user ${userId}, service ${service}, app ${appName}`);
      ...
      const connectionRequest = await entity.initiateConnection({
        appName: appName,
        redirectUrl: process.env.COMPOSIO_REDIRECT_URL || `${process.env.DOMAIN_CLIENT}/api/composio/callback?userId=${userId}&service=${service}`,
      });
      ...
      logger.info(`[ComposioService] Connected Account ID: ${connectionRequest.connectedAccountId}`);
      ...
      logger.error(`[ComposioService] Failed to initiate connection for user ${userId}, service ${service}:`, error);
      throw error;
    }
  }
  ...
  async waitForConnectionActive(userId, service, connectedAccountId, timeoutSeconds = 180) {
    ...
    try {
      logger.info(`[ComposioService] Waiting for connection ${connectedAccountId} to become active...`);
  ...
  async completeOAuthFlow(userId, service, connectedAccountId) {
    ...
    try {
      ...
      logger.info(`[ComposioService] Completed OAuth flow for user ${userId}, service ${service}, connected account: ${connectedAccountId}`);
    } catch (error) {
      logger.error(`[ComposioService] Failed to complete OAuth flow for user ${userId}, service ${service}:`, error);
      throw error;
    }
  }
}
