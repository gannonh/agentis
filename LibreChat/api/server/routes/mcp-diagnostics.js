  ...
  try {
    ...
    for (const [userId, serverMap] of userConnections.entries()) {
      ...
      for (const [serverName, connection] of serverMap.entries()) {
        ...
        logger.info(`[MCP-DIAGNOSTICS] User ${userId} - Server ${serverName} - Connected: ${isConnected}`);
...
async function diagnoseSpecificUser(userId, logger) {
  try {
    ...
    if (!userConnections || !userLastActivity) {
      logger.info(`[MCP-DIAGNOSTICS] Unable to access user connections or activity maps for user ${userId}`);
      return null;
    }
    ...
    for (const [serverName, connection] of userServerMap.entries()) {
      ...
      logger.info(`[MCP-DIAGNOSTICS] User ${userId} - Server ${serverName} - Connected: ${isConnected}, State: ${state}`);
    }
