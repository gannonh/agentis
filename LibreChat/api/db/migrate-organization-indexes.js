...
async function preMigrationChecks() {
  ...
  try {
    ...
    for (const [collection, stats] of Object.entries(analysis)) {
      logger.info(`   ${collection}: ${stats.documentCount} documents (${(stats.totalSize / 1024 / 1024).toFixed(2)} MB)`);
    }
    ...
    for (const [collection, stats] of Object.entries(analysis)) {
      ...
      const plannedIndexes = ['organization_slug_unique', 'organization_domain_lookup', 'member_user_org_unique', 'invitation_expiration_ttl'];
    ...
    logger.info(`⏱️  Estimated migration time: ${estimatedMinutes} minute(s) for ${totalDocs} total documents`);
...
async function postMigrationVerification() {
  ...
  try {
    ...
    const expectedIndexes = {
      organization: ['organization_slug_unique', 'organization_domain_lookup', 'organization_created_analytics'],
      member: ['member_user_org_unique', 'member_user_lookup', 'member_org_role_lookup', 'member_org_lookup'],
      invitation: ['invitation_org_status_lookup', 'invitation_email_status_lookup', 'invitation_expiration_ttl', 'invitation_org_lookup']
    };
    ...
    if (allIndexesPresent) {
      ...
      logger.info(`   • Organization domain lookup: ${PERFORMANCE_TARGETS.organizationLookupByDomain}`);
      logger.info(`   • User membership checks: ${PERFORMANCE_TARGETS.userMembershipCheck}`);
      logger.info(`   • Organization member listing: ${PERFORMANCE_TARGETS.organizationMemberList}`);
...
async function runMigration() {
  ...
  try {
    ...
    if (verificationPass && results.errors.length === 0) {
      ...
      logger.info('   1. Monitor query performance with: node migrate-organization-indexes.js analyze');
...
async function analyzeDatabaseState() {
  ...
  try {
    ...
    for (const [collection, stats] of Object.entries(analysis)) {
      ...
      if (stats.indexes.length > 1) { // Skip default _id index
        stats.indexes.forEach(idx => {
          if (idx.name !== '_id_') {
            ...
            console.log(`      • ${idx.name}: ${JSON.stringify(idx.key)} ${features.length > 0 ? `[${features.join(', ')}]` : ''}`);
          }
        });
      } else {
    ...
    QUERY_PATTERNS.mediumFrequency.forEach(pattern => {
      console.log(`   • ${pattern.collection}: ${JSON.stringify(pattern.query)} - ${pattern.description}`);
    });
    ...
    const totalDocs = Object.values(analysis).reduce((sum, stats) => sum + stats.documentCount, 0);
    const totalIndexes = Object.values(analysis).reduce((sum, stats) => sum + stats.indexes.length - 1, 0); // Exclude _id indexes
