...
import { promptGroupSchema, promptSchema } from '@librechat/data-schemas';
import { escapeRegExp } from '#server/utils/index.js';
import { logger } from '#config/index.js';

const PromptGroup = mongoose.model('PromptGroup', promptGroupSchema);
const Prompt = mongoose.model('Prompt', promptSchema);

// Export Prompt as default since it's the primary model
 ...
 * @returns {Promise<TCreatePromptResponse>}
 */
const savePrompt = async (saveData) => {
  try {
 ...
 */
const getRandomPromptGroups = async (filter) => {
  try {
...
const getPromptGroupsWithPrompts = async (filter) => {
  try {
...
const getPromptGroup = async (filter) => {
  try {
 ...
 */
const deletePrompt = async ({ promptId, groupId, author, role }) => {
 ...
 */
const updatePromptGroup = async (filter, data) => {
  try {
    const updateOps = {};
 ...
 */
const makePromptProduction = async (promptId) => {
  try {
...
const updatePromptLabels = async (_id, labels) => {
  try {
