...
vi.mock('~/hooks', () => ({
  useLocalize: () => (key: string, args?: { [key: string]: unknown }) => {
    ...
    if (key === 'com_ui_select_tools_description') return 'Select the tools you want to add to your agent';
    if (key === 'com_ui_select_all_tools') return 'Select All';
    if (key === 'com_ui_helper_tools') return 'Helper tools';
    if (key === 'com_ui_helper_tools_description') return 'Helper tools will be included automatically';
...
describe('MCPServerToolSelect', () => {
  ...
  it('renders correctly when open', () => {
    ...
    expect(screen.getByText('Helper tools')).toBeInTheDocument();
  ...
  it('calls onConfirm with selected tools when Add Selected is clicked', () => {
    ...
    const firstToolCheckbox = screen.getByText('Create Sheet').previousSibling as HTMLInputElement;
