...
export const OrganizationDetectionDisplay: React.FC<OrganizationDetectionDisplayProps> = ({
  ...
  if (isLoading) {
    return (
      <div
        className={`flex items-center gap-2 rounded-md border border-gray-300 bg-gray-50 px-3 py-2 text-sm text-gray-600 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-300 ${className}`}
      >
  ...
  if (organization) {
    return (
      <div
        className={`rounded-md border border-blue-300 bg-blue-50 px-3 py-2 text-sm text-blue-700 dark:border-blue-700 dark:bg-blue-900/20 dark:text-blue-300 ${className}`}
      >
  ...
  if (isNewDomain) {
    return (
      <div
        className={`rounded-md border border-green-300 bg-green-50 px-3 py-2 text-sm text-green-700 dark:border-green-700 dark:bg-green-900/20 dark:text-green-300 ${className}`}
      >
        <p className="font-medium">
          You&apos;ll create a new organization for <strong>{domain}</strong>
        </p>
        <p className="mt-1 text-xs opacity-80">You&apos;ll be the organization owner</p>
      </div>
    );
  }
