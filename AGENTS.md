...
- **Thread UI:** official [AI Elements](https://elements.ai-sdk.dev) in `apps/web/src/components/ai-elements/`; thread session in `apps/web/src/hooks/use-thread-session.ts`. Transcript rendering uses `thread-transcript.tsx` (turn grouping), `thread-message-content.tsx` with `native-tool-display.ts` for tool-result cards, and `message-text.ts` (shared heuristic in `packages/shared`) to avoid raw provider JSON when tool-result parts exist. **Working artifacts** rail on `/threads/:threadId` (`thread-durable-artifacts.tsx`): lists thread-scoped Library artifacts, inline document/static preview in the right rail, collapsed mobile bar above the transcript.
