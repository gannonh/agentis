...
function inferSearchQuery(prompt: string): string {
  const normalized = prompt
    .replace(/\b(search|look up|find|research|investigate)\b/gi, " ")
...
export async function executeMockResearchBriefStream(
  ...
  if (!deps.documentService) {
    throw new Error("Document service is required for mock research brief runs")
  }
  ...
  const searchToolName = "searchWeb" as const
  const searchToolCallId = `mock-native-search-${runId}`
  const searchToolInput = { query: inferSearchQuery(latestUserPrompt) }
  let searchToolOutput: SearchWebOutput
  try {
    searchToolOutput = await deps.webSearchService.search(searchToolInput)
  } catch (error) {
  ...
  const title = inferResearchBriefTitle(latestUserPrompt)
  const sourceLines = searchToolOutput.results
    .map((result) => `- [${result.title}](${result.url})`)
    .join("\n")
  const generated = deps.documentService.registerGenerated({
    title,
    description: "Research brief generated by mock runtime",
    content: `# ${title}\n\n## Summary\n\n${formatMockWebSearchSummary(searchToolOutput)}\n\n## Sources\n\n${sourceLines || "_No sources returned._"}\n\n## Request\n\n${latestUserPrompt.trim()}`,
    projectId: thread.projectId ?? undefined,
    threadId: thread.id,
    runId,
  })
  if (!generated.ok) {
    ...
    deps.repos.steps.create({
      ...
      payload: { code: generated.code, message },
    })
    deps.repos.threads.touch(run.threadId, { status: "failed" })
    throw new Error(message)
  }

  const summaryText = `Found ${searchToolOutput.resultCount} web sources and saved **${generated.document.title}** to the Library.`
  const assistantParts: MessagePart[] = [
    { type: "text", text: summaryText },
    {
      type: "tool-call",
  ...
  for (const [toolName, toolCallId, toolInput, toolOutput] of [
    [searchToolName, searchToolCallId, searchToolInput, searchToolOutput],
  ] as const) {
    deps.repos.steps.create({
      runId,
      type: "tool-call",
