...
export class RunExecutor {
  ...
  async executeStream(runId: string) {
    ...
    let mockDocumentSuffix = ""
    if (this.config.mockRuntime && wantsGeneratedDocument(latestUserPrompt)) {
      const runContext = {
        threadId: thread.id,
        projectId: thread.projectId ?? undefined,
        runId,
      }
      const globalDocument = wantsGlobalDocument(latestUserPrompt)
      const requestedTitle = requestedDocumentTitle(latestUserPrompt)
      if (wantsDocumentUpdate(latestUserPrompt)) {
        const target = this.documentService
          .findDocuments({
        ...
        const updated = target
          ? this.documentService.appendDocumentSection({
              documentId: target.id,
              heading: "Mock runtime update",
              content: `Updated from ${thread.title}: ${latestUserPrompt.trim()}`,
      ...
      if (!mockDocumentSuffix) {
        const title = mockDocumentTitle({ requestedTitle, globalDocument })
        let projectId: string | undefined
        if (!globalDocument) {
          projectId = thread.projectId ?? undefined
        }
        const generated = this.documentService.registerGenerated({
          title,
