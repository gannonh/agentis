...
function wantsGeneratedDocument(prompt: string) {
  return DOCUMENT_PROMPT_PATTERN.test(prompt)
}

function wantsDocumentUpdate(prompt: string) {
  return (
...
function wantsGlobalDocument(prompt: string) {
  return /global|workspace-wide|workspace wide/i.test(prompt)
}

function requestedDocumentTitle(prompt: string) {
  ...
  async executeStream(runId: string) {
    ...
    if (this.config.mockRuntime && wantsGeneratedDocument(latestUserPrompt)) {
        ...
        const target = this.documentService
          .findDocuments({
            query: requestedTitle ??
              (wantsGlobalDocument(latestUserPrompt) ? undefined : latestUserPrompt),
            runContext,
            limit: 10,
          })
      ...
      if (!mockDocumentSuffix) {
        ...
        const title = requestedDocumentTitle(latestUserPrompt) ??
          (globalDocument ? "Global playbook" : "Project report")
        const generated = this.documentService.registerGenerated({
          ...
          projectId: globalDocument ? undefined : (thread.projectId ?? undefined),
          threadId: thread.id,
          runId,
        })
        if (generated.ok) {
