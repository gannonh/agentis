...
export function isNativeAppToolName(
  toolName: string
): toolName is NativeAppToolName {
  return NATIVE_APP_TOOL_NAMES.includes(toolName as NativeAppToolName)
}
  ...
  return {
    ...
    previousVersion:
      typeof output.previousVersion === "number"
        ...
        : undefined,
    visibilityScope:
      typeof output.visibilityScope === "string"
        ? output.visibilityScope
        : undefined,
    viewPath:
      typeof output.viewPath === "string"
        ? truncateText(output.viewPath, STATIC_ARTIFACT_TEXT_MAX_CHARS)
    ...
    summary:
      typeof output.summary === "string"
        ? truncateText(output.summary, STATIC_ARTIFACT_TEXT_MAX_CHARS)
        : undefined,
    errorCode:
      typeof output.errorCode === "string"
        ? truncateText(output.errorCode, STATIC_ARTIFACT_TEXT_MAX_CHARS)
...
function summarizeAppInput(input: unknown): unknown {
  if (!isObject(input)) return input
  return {
    title:
      typeof input.title === "string"
...
export function formatNativeToolRunStepPayload(input: {
  ...
  if (isNativeAppToolName(input.toolName)) {
    return {
      provider: "native",
      toolCallId: input.toolCallId,
      toolName: input.toolName,
      input: summarizeAppInput(input.input),
      output:
        input.output === undefined
          ? undefined
          : summarizeAppOutput(input.output),
      error: input.error,
      code: input.code,
    }
  }
