...
import { artifactTypeSchema, listArtifactsQuerySchema } from "@workspace/shared"
...
export function createArtifactRoutes(repos: Repositories, config: AppConfig) {
  ...
  app.post("/", async (c) => {
    ...
    if (!parsedType.success) {
      return c.json(
        { error: "Invalid artifact type", code: "invalid_request" },
        400
      )
    }
  ...
  app.get("/:artifactId/download", (c) => {
    ...
    const filename =
      result.artifact.title.replace(/[^\w.-]+/g, "_") || "artifact"
