...
export function createMemoryRoutes(repos: Repositories): Hono {
  ...
  function validateAgentScope(input: {
    ...
    const agentIds = input.associatedAgents?.length
    ...
    const missingAgent = agentIds.find((agentId) => !repos.agents.getById(agentId))
    if (missingAgent) {
  ...
  app.patch("/:id", async (c) => {
    ...
    const nextAssociatedAgents =
      nextScope === "agent"
        ? input.associatedAgents?.length
          ? input.associatedAgents
          : input.associatedAgent
            ? [input.associatedAgent]
            : existing.associatedAgents
        : []
