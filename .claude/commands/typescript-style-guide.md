...
📏 Rule: `'@typescript-eslint/ban-ts-comment': ['error', { 'ts-expect-error': 'allow-with-description' }]`
...
Define all types using type alias 📏 Rule: `'@typescript-eslint/consistent-type-definitions': ['error', 'type']`
...
Array types should be defined using generic syntax 📏 Rule: `'@typescript-eslint/array-type': ['error', { default: 'generic' }]`
...
Explicitly defining the return type of a function is encouraged, although not required 📏 Rule: `"@typescript-eslint/explicit-function-return-type": "error"`
...
📏 Rule: `'no-restricted-syntax': ['error', { selector: 'TSEnumDeclaration', message: 'Replace enum with a literal type or a const assertion.' }]`
...
Named exports must be used to ensure that all imports follow a uniform pattern 📏 Rule: `'import/no-default-export': 'error'`
...
* **Booleans**
  ...
  📏 Rule: `'@typescript-eslint/naming-convention': [{ selector: 'variable', types: ['boolean'], format: ['PascalCase'], prefix: ['is', 'are', 'should', 'has', 'can', 'did', 'will'] }]`
...
📏 Rule: `'@typescript-eslint/naming-convention': [{ selector: 'typeAlias', format: ['PascalCase'] }]`
...
📏 Rule: `'@typescript-eslint/naming-convention': [{ selector: 'typeParameter', format: ['PascalCase'], custom: { regex: '^T[A-Z]', match: true } }]`
...
📏 Rule: `'react/jsx-handler-names': ['error', { eventHandlerPrefix: 'handle', eventHandlerPropPrefix: 'on' }]`
...
Camel case, prefixed as 'use' 📏 Rule: `'react-hooks/rules-of-hooks': 'error'`

Symmetrically convention as [value, setValue] = useState() 📏 Rule: `'react/hook-use-state': 'error'`
...
📏 Rule: `'vitest/valid-title': ['error', { mustMatch: { it: [/should.*when/u.source, "Test title must include 'should' and 'when'"] } }]`
