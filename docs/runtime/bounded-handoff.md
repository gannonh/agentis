...
The workspace also contains an immutable `.cursor/hooks.json` with a `subagentStart` command hook. Its fixed `/bin/sh` script returns `permission: deny`; `failClosed: true` requests denial if the script fails or exceeds five seconds. Agentis verifies the hook configuration and script on launch and load. This uses Cursor's documented [subagentStart hook](https://cursor.com/docs/hooks#subagentstart). The live native enforcement probe FAILED: the pinned ACP process launched a synthetic Task despite this configuration. A separate native configuration-loader check confirmed readable, valid project hooks. No supported correction was identified; the precise runtime cause remains undetermined. See the [acceptance evidence](../verification/kat-3243/README.md). This candidate remains draft until the required enforcement is established.
