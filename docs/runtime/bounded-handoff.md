...
The workspace also contains an immutable `.cursor/hooks.json` with a `subagentStart` command hook. Its fixed `/bin/sh` script returns `permission: deny`; `failClosed: true` also blocks subagent startup if the script fails or exceeds five seconds. Agentis verifies the hook configuration and script on launch and load. This uses Cursor's documented [subagentStart hook](https://cursor.com/docs/hooks#subagentstart). Native live enforcement still requires a probe that actually attempts Task/subagent dispatch; schema and script tests alone do not establish it.
