...
The final browser run passed at source commit `6035f90c950af50a5602c0139c67afabe98f57f3` with a clean source tree. The later evidence/documentation commit does not change the tested prototype. [Raw results](evidence/results.json) contain SHA-256 hashes of every source/protocol file and the full action log. Start and verification commands are in the [walkthrough](README.md#reproduce-the-browser-evidence).
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| A, shared team room | 10 | 3 | 0 | 5 | 4 | 10 | 40 |
| B, manager DM | 10 | 3 | 0 | 5 | 4 | 10 | 42 |
...
Provider/execution selection and source authority now precede request submission and task creation. Destination account/project connection remains the third setup step. Setup starts no task or specialist. The destination connection remained separate from exact-action approval. The prototype does not implement a second connected task; omitting those setup steps on later work is an expectation, not a measured repeat-use result.
...
| --- | ---: | ---: | ---: | ---: | ---: |
| A desktop | 24.25 | 22.32 | 24.62 | 22.67 | 25.90 |
| B desktop | 26.30 | 25.06 | 25.29 | 24.84 | 23.84 |
| C desktop | 29.10 | 26.95 | 32.66 | 28.46 | 34.06 |
| A narrow keyboard | 24.47 | 23.47 | 23.44 | 22.84 | 23.22 |
| B narrow keyboard | 22.44 | 23.21 | 24.37 | 25.21 | 21.78 |
| C narrow keyboard | 25.30 | 26.73 | 27.80 | 25.33 | 31.36 |
...
| Assignment, accepted handoff, clarification, artifact, exact approval, receipt, interrupted/unknown | PASS for simulated presentation and transitions in each alternative. The driver records 231 successful checks across six runs. |
...
| Conversation assignment and explicit setup/authority | PASS in simulation. No roster, builder, source-file configuration, or credential input. Connection and source authority precede the first request and task creation. Three setup activations are counted separately. |
...
Preflight found and corrected an old-message scroll position, a hidden clarification in B, an omitted C navigation count, an incorrect “approval required” label after approval, and permission-duration wording that contradicted the flow. Browser checks found a dialog focus-cycle issue and inconsistent restored focus indication. The driver initially checked focus before the asynchronous close event; it now waits for actual restoration. A read-only reviewer also identified lost focus on C navigation. PR review also exposed early task creation, premature specialist work updates, incorrect dialog opener restoration, and source attribution that trusted an unrelated server. The final run verifies the corrected onboarding order, exact opener identity, and browser-consumed source hashes. A subsequent queued-dismissal focus race was reproduced and corrected with synchronous focus restoration. The six [source-attribution regressions](evidence/source-binding.json) all rejected the injected mismatch or redirect. [Preflight findings](preflight.md) preserve the failures and scope of corrections.
