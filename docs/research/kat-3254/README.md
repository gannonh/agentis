...
1. In C, choose **Start a request**. A and B already show the composer.
2. Choose **Send request** to ask Mara for the supplied release brief. The first task starts in conversation without roster or workflow configuration. Only the supplied scenario text is accepted.
3. Choose **Use demonstration environment**, then **Allow this source read**. These separate the fictional provider/execution choice from task-scoped source authority.
...
The recorded run used Chromium at `/usr/bin/chromium`. Set `CHROMIUM_PATH` for another installed Chromium binary. Set `PLAYWRIGHT_MODULE` to an absolute Playwright `index.mjs` path if installed elsewhere. `PROTOTYPE_URL` changes the explicit loopback origin. The driver uses fresh browser contexts, permits only that origin, and writes screenshots and `results.json` to `EVIDENCE_DIR`.
