...
The selected providers are Codex app-server and direct Claude Agent SDK. Product source: `3c76c681ecd1be59789311d3fb3dd7ab3d555bfe`. Verification ran on macOS arm64, Docker Desktop and Node 24.20.0 on September 7, 2026. All nine Build acceptance criteria pass with the evidence distinctions below. The PR is ready for Agent Review; this does not grant merge permission.
...
| AC6: Decisions and failures | PASS | Both live allow, deny, input, cancel and load cases passed. Missing-auth evidence is live; quota and crash handling use deterministic protocol fixtures. No real account quota was exhausted. |
...
| AC9: Fixtures plus real proof | PASS | 96 tests pass, covering malformed/error and lifecycle cases, alongside the selected providers’ live workflows and combined handoff/concurrency proof. |

## Verification

All required checks passed: frozen dependency install, typecheck, lint, 96 tests across 12 files, build and packaged CLI smoke. [Captured checks](evidence/logs/claude-final-checks.log) and [independent spec/quality review](replacement-provider/implementation-review.json) record the results.
...
- [Claude missing auth](evidence/responses/provider-conformance-ivo-no-auth.json) and [Codex missing auth](evidence/responses/provider-conformance-mara-no-auth.json): typed launcher failures with no artifact. Each receipt names its own source revision.
