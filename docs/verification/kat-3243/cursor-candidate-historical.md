> Historical rejected Cursor candidate. Current acceptance is in [README](README.md). Status statements below describe that earlier candidate.

# Two-provider workflow evidence

Provider-selection revision: Gannon approved deferring Cursor and testing a replacement before adapter work. The matrix below records the rejected Codex/Cursor candidate; its passing rows do not certify a replacement. The [replacement feasibility record](replacement-provider/README.md) now passes all three live cases with Claude SDK; full adapter acceptance is pending. Codex and the lifecycle fixes remain in the draft.
...
Both real-container provider launch paths passed missing-auth checks: [Codex](evidence/responses/provider-conformance-mara-no-auth.json) and [Cursor](evidence/responses/provider-conformance-cursor-no-auth-historical.json). These prove the unavailable-auth path only. Codex refuses the missing credential in the container launcher before executing the provider binary; this is not a native Codex authentication response. The owner subsequently completed native Codex login successfully; [authentication receipt](evidence/responses/codex-authentication.json). Authentication is resolved for both providers.
