...
**Fail for live user sign-off (re-opened 2026-05-22)** — automated/mock verification at `2165a6ea` did not catch: (1) Playwright test API hijacking port 3001 with `AGENTIS_MOCK_COMPOSIO=1`, (2) stale `pending` connections blocking Connect in the UI, (3) incomplete live OAuth. Treat **Verified** as withdrawn until a human completes Connect → OAuth callback → grant → tool run on live Composio without demo mode.
...
| Pass (partial) | Connect in live Composio mode                            | `POST /api/integrations/github/connect` → `201`, `redirectUrl` host `backend.composio.dev`, `composioMockEnabled: false` (API on `:3002`, `AGENTIS_MOCK_COMPOSIO=0`). **Pending:** complete OAuth in browser and confirm `connected` status. |
| Pass           | Five featured toolkits, API-backed status and search     | `apps/api/src/routes/integrations.test.ts`; `apps/web/src/routes/integrations.vitest.tsx`                                                                                                                                                    |
...
**Important:** Complete live OAuth once (Integrations → Connect GitHub → follow Composio redirect → land back on `/integrations?connected=github`). A dev server started earlier with `AGENTIS_MOCK_COMPOSIO=1` will keep reporting `reason: mock_enabled` until restarted without mock flags.
...
### Live Composio smoke (2026-05-22)

Prerequisites in repo root `.env`:

- `COMPOSIO_API_KEY` (from [composio.dev](https://composio.dev) → Settings → Project Settings → API Keys)
- `COMPOSIO_REDIRECT_BASE_URL=http://127.0.0.1:3001` (must match the API port Composio redirects to)
- Do **not** set `AGENTIS_MOCK_COMPOSIO=1` for live mode
...
Observed with live client:

- `GET /api/runtime/health` → `composio.available: true`, no `mock_enabled`
- `GET /api/integrations` → `composioMockEnabled: false`, five toolkits listed
- `POST /api/integrations/github/connect` → `201`, redirect to `backend.composio.dev`

### Recommendation

**Pending user sign-off** — mock + automated gates are green; live verification is real once you complete OAuth and a granted-tool thread run. Restart any long-running `pnpm dev` / API process so it picks up `.env` without mock Composio.