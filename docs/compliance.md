...
The September 11, 2026 [KAT-3247 verification](verification/kat-3247/README.md) exercised product commit `e2085e98c27d530b0c9e66224d07e853f68f5f17` on macOS Apple silicon with Docker Desktop. Codex app-server and direct Claude Agent SDK each passed the six live scratch cases. Gate 0 remains UNVERIFIED because the gate report retains incomplete rejection and isolation evidence. Gate 1 remains blocked.
...
| Claude local | Direct Agent SDK 0.3.263, native CLI 2.1.263, scoped Anthropic API-key auth, model `claude-sonnet-5`, medium effort | Selected. Live scratch output, allow, deny, AskUserQuestion plan rejection, structured input, cancel and history loading passed. Accepted specialist handoff and concurrent stop-all passed. Live rejected-handoff ownership remains unverified. API usage is billed through the provisioned Anthropic account; this run did not reconcile invoices or certify subscription access. |
...
Provider-session loading is history loading, not interrupted-action recovery. MCP and attachments remain unavailable. Fixture success, core CI and packaged smoke do not establish credential, absolute-executable or network isolation of the host fixture launcher. The [gate matrix](verification/kat-3247/README.md) records those distinctions and every requirement verdict.
