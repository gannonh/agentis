...
Use this skill when an agent needs evidence from the running Agentis CLI/daemon. Work from the repository root. The recipes use the public CLI and `/v1/*` API; they do not open `state.sqlite` or call engine internals. The source contracts are [cli.ts](../../../packages/cli/src/cli.ts), [verify.ts](../../../packages/cli/src/verify.ts), [http.ts](../../../packages/cli/src/http.ts), and [schema.ts](../../../packages/cli/src/schema.ts). Read the [feature map](./features/README.md) before selecting a recipe.
...
Keep the exact `endpoint`, `dataRoot`, `workspace`, `log`, and daemon `pid` from that object. The port is never guessed. `workspace` is the daemon scratch root; each task artifact is below `workspace/runs/<runId>/`. The daemon is ready when the object has been emitted and `GET /v1/health` returns JSON with `ok: true`. `verify launch` has no live provider support and does not establish a Gate 0 or provider eligibility claim.

For a manual run, follow the [manual session reference](./references/manual-session.md) to capture readiness, bind the shell variables, and verify process identity before control commands. Press `Ctrl-C` in the launcher terminal after the drive, or let the smoke helper own the launcher lifecycle. Do not start a second `serve` process against the same data root.
...
If the precheck fails, stop and clean up that launch. Do not run `doctor` to repair the credential. A profile is available at `$DATA_ROOT/profiles/verify.json`, but explicit `--endpoint` and `--data-root` keep the instance under review visible and avoid selecting an unrelated default root.
...
Every verification launch owns its temporary data root and must be stopped in a `finally` path. The readiness `pid` is the daemon PID; the foreground `verify launch` process has a separate `LAUNCHER_PID`. For exact manual launch, PID identity, bounded wait, evidence persistence, and removal commands, use [the manual session reference](./references/manual-session.md). If a run is waiting, issue `run cancel` for that run first; use `stop-all` when the intended proof is global cancellation. The helper owns the same launcher/daemon wait and removes only its validated temporary root.
...
The supplied `EVIDENCE_DIR` must not already exist. The helper contract is deliberately small: create and own the temporary launch parent, spawn the foreground `verify launch`, parse the readiness object, pre-read and validate `owner.token` without printing it, run `doctor`, submit the smoke fixture with brief `verification-smoke`, fetch authenticated public status, validate the owned artifact bytes and hash against `# verification-smoke\n`, write redacted evidence, SIGTERM the launcher, await launcher exit and daemon disappearance, and remove only the owned temp root. It must return non-zero on any assertion or cleanup failure and retain evidence on failure.
