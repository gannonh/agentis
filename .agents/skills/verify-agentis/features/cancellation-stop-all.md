...
- **Prove the latch.** Run `node packages/cli/dist/bin.js task submit --endpoint "$ENDPOINT" --data-root "$DATA_ROOT" --brief "KAT-3315 after stop all" --fixture smoke > "$EVIDENCE_DIR/after-stop-submit.json"`. Require exit code `1`, an HTTP `409`-derived receipt with `accepted: false`, and an error containing `stop-all is latched`.
