...
- **Propose.** With a valid `MARA_RUN_ID`, run `node packages/cli/dist/bin.js task handoff --endpoint "$ENDPOINT" --data-root "$DATA_ROOT" --run "$MARA_RUN_ID" --brief "Return the bounded draft" > "$EVIDENCE_DIR/handoff-propose.json"`. The public `POST /v1/commands` body is `{ "idempotencyKey": "<fresh unique key>", "command": { "kind": "propose_handoff", "sourceRunId": "$MARA_RUN_ID", "recipient": "ivo", "context": "Return the bounded draft" } }`; retain the exact outer key and command in evidence.
